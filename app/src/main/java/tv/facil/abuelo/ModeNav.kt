...
/**
 * Color-button navigation and resume helpers (TV / Series / Movies / Settings).
 *
...
object ModeNav {
    ...
    private fun currentScreenFor(activity: Activity): AppScreen = when (activity) {
        ...
        is PlayerActivity -> if (activity.isLiveZap) AppScreen.TV else AppSettings.lastScreen
    ...
    fun leaveSettings(activity: AppCompatActivity, sourceId: String) {
        when (AppSettings.settingsReturnScreen) {
    ...
    suspend fun resolveLiveChannel(context: Context, sourceId: String): CatalogItem? =
        withContext(Dispatchers.IO) {
            ...
            if (last != null) {
                playable.find { it.url == last.url || (last.streamId != null && it.streamId == last.streamId) }
                    ?: playable.first()
            } else {
                playable.first()
            }
